...
class TenantsController extends Controller
{
    ...
    public function update(UpdateUsersRequest $request, $id)
    {
        ...
        $tenant = User::findOrFail($id);
       // $tenant->update($request->all()+['fecha_fin_contrato' => strtotime ( '-90 day' , strtotime ( $fecha_fin_contrato))
        $tenant->
        update($request->all());
            //+
            //['fecha_fin_contrato' => $fecha_fin_contrato]);
        //);
        //strtotime ( '-90 day' , strtotime ( $fecha_fin_contrato))

        return redirect()->route('admin.tenants.index')
        ->with('flash','Inquilino actualizado correctamente');
    }
    ...
    $facturas = \App\PropertiesFacturas::where('id_tenant', $id)->get();
